...
@pytest.mark.telemetry_files('Av_test.py')      
def test_Assembly():
            ...
            if ((line.strip()[0] != '#') and ( line.strip().find('.nasm') > 0 )):
                ...
                if os.path.isfile(mif):
                    ...
                    for test in range(0, int(par[1])):
                        ramIn = "tests/nasm/" + name + "/" + name +"{}".format(test)+ "_in.mif"
                        ramTest = "tests/nasm/" + name + "/" + name +"{}".format(test)+ "_tst.mif"
...
def test_VM():
    ...
    file_test = open("config_testes_vm.txt","r")
    lines_test = file_test.readlines() 
    file_test.close()
    ...
    for line in lines_test:
        if len(line.strip()):
            if ((line.strip()[0] != '#') and ( line.strip().find('.vm') > 0 )):
                # pega parametros e atribui caminhos globais
                # par[0] : Nome do teste (subpasta)
                # par[1] : quantidade de testes a serem executados
                # par[2] : tempo de simulação em ns
                ...
                if os.path.isfile(mif):
                    # simulate
                    for test in range(0, int(par[1])):
                        ramIn = "tests/vm/" + name + "/" + name +"{}".format(test)+ "_in.mif"
                        ramTest = "tests/vm/" + name + "/" + name +"{}".format(test)+ "_tst.mif"
